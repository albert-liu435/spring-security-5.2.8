...
public class RunAsManagerImpl implements RunAsManager, InitializingBean {
	...
	/**
	 * 如果受保护对象对应的 ConfigAttribute 中拥有以“RUN_AS_”开头的配置属性，则在该属性前加上“ROLE_”，然后再把它作为一个 SimpleGrantedAuthority 赋给将要创建的 Authentication
	 * （如ConfigAttribute 中拥有一个“RUN_AS_ADMIN”的属性，则将构建一个“ROLE_RUN_AS_ADMIN”的SimpleGrantedAuthority），最后再利用原 Authentication 的 principal、权限等信息构建一个新的 Authentication 并返回；
	 * 如果不存在任何以“RUN_AS_”开头的 ConfigAttribute，则直接返回null。
	 *
