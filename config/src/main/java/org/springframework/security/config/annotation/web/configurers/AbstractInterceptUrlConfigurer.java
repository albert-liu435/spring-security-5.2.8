...
/**
 * https://andyboke.blog.csdn.net/article/details/93350686
 * <p>
...
abstract class AbstractInterceptUrlConfigurer<C extends AbstractInterceptUrlConfigurer<C, H>, H extends HttpSecurityBuilder<H>>
	...
	/**
	 * 抽象方法，要求子类提供实现。用于创建安全元数据对象FilterInvocationSecurityMetadataSource,供目标安全拦截过滤器FilterSecurityInterceptor使用。
	 * 是由 AbstractInterceptUrlConfigurer 类的子类 ExpressionUrlAuthorizationConfigurer 提供
	...
	/**
	 * 该方法被configure用于获取最终用于目标FilterSecurityInterceptor要使用的AccessDecisionManager。如果调用者设置了当前安全配置器的AccessDecisionManager属性则使用它，
