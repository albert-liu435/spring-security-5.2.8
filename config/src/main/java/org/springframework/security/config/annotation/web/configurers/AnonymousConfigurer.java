...
/**https://andyboke.blog.csdn.net/article/details/91391002
 * 作为一个配置HttpSecurity的SecurityConfigurer,AnonymousConfigurer的配置任务如下 :
 * <p>
 * 配置如下安全过滤器Filter
 ...
 * <p>
 * 缺省情况下，AnonymousConfigurer 是被启用的，也就是说，即使用户不登录访问某个页面，SecurityContextHolder中也会持有一个Authentication，
 * 只不过这个Authentication是一个AnonymousAuthenticationToken。这么做的目的主要是为了方便使用者，即使用户未登录，使用者从SecurityContextHolder获得的Authentication也不会是null。而在概念和语义上，"匿名认证用户"和"未认证的用户"这二者并无区别。
