...
/**
 * https://andyboke.blog.csdn.net/article/details/93376098
 * <p>
 * 作为一个配置HttpSecurity的SecurityConfigurer,ExpressionUrlAuthorizationConfigurer的配置任务如下 :
 * <p>
 * 配置如下安全过滤器Filter
 * FilterSecurityInterceptor
 * ExpressionUrlAuthorizationConfigurer可以为多组 RequestMatcher 分别配置不同的权限属性。这里每组RequestMatcher表示一组调用者想设定成相同权限控制的Http method/URL pattern,
 * 这里所设置的权限属性其实是基于SpEL的权限表达式。ExpressionUrlAuthorizationConfigurer可以接收一个调用者指定的安全表达式处理器来理解这些权限表达式。如果调用者不指定，则使用缺省的安全表达式处理器
 * DefaultWebSecurityExpressionHandler来理解这些权限表达式。
 * <p>
 * ExpressionUrlAuthorizationConfigurer继承自AbstractInterceptUrlConfigurer。作为一个安全配置器，它们对目标安全构建器HttpSecurity的主要配置逻辑实现在AbstractInterceptUrlConfigurer#configure。
