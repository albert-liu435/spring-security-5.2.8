...
/**
 * 用于配置AbstractAuthenticationFilterConfigurer,仅供内部使用
...
public abstract class AbstractAuthenticationFilterConfigurer<B extends HttpSecurityBuilder<B>, T extends AbstractAuthenticationFilterConfigurer<B, T, F>, F extends AbstractAuthenticationProcessingFilter>
	...
	/**
	 * 默认登陆成功后跳转到此 ，如果 alwaysUse 为 true 只要进行认证流程而且成功，会一直跳转到此。一般推荐默认值 false
	 * Specifies where users will be redirected after authenticating successfully if
	...
	/**
	 * 指定验证凭据的URL。
	 * 实际表单向后台提交用户信息的 Action，再由过滤器UsernamePasswordAuthenticationFilter 拦截处理，该 Action 其实不会处理任何逻辑。
	 * 即表单提交的url地址
	...
	/**
	 * 自定义认证成功处理器，可替代上面所有的 success 方式
	...
	/**
	 * 设置认证失败处理器，默认采用的为SimpleUrlAuthenticationFailureHandler
	...
	/**
	 * 设置登录页，并更新AuthenticationEntryPoint
